...
export class CustomizeGameService {
  ...
  constructor(
    ...
    this.arePlayersReadyRule = new ArePlayersReadyRule(this.waitingQueue);

    this.startGameRule = new StartGameRule(this.waitingQueue, this.gameManager);
  }
  ...
  public async execute(request: Request): Promise<void> {
    try {
      ...
      console.log('arePlayersReadyRule');
