...
export class CanPlayersPlayRule {
  ...
  public async apply(request: Request): Promise<boolean> {
    if (
      (await this.gameHistory.getRunningGameByPlayerId(
    ...
    if (
      (await this.WaitingQueue.isOnQueue(request.message.data.from)) != false
    ) {
      return false;
    }

    if (
      (await this.invitationRepository.hasOpenInvite(
        request.message.data.from,
        request.message.data.to,
      )) != false
    ) {
